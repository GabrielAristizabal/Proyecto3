...
public class LectorEscritorArchivos {
	...
	private ArrayList<CaracteristicasAdicionales> caracteristicasHotel;
	...
	private File archivoCaracteristicasHotel = new File("data/caracteristicasHotel.txt");
	private File archivoCaracteristicasHabi = new File("data/caracteristicasHabi.txt");
	...
	public LectorEscritorArchivos(ArrayList<Comsumible> servicios, ArrayList<Producto> menu,
			...
			HashMap<String, String> registroConsumos, ArrayList<CaracteristicasAdicionales> caracteristicasHotel) {
	...
	private void cargarCaracteristicasHabi() throws IOException {
		BufferedReader br = new BufferedReader(new FileReader(archivoCaracteristicasHabi));
		String linea = br.readLine();
		while (linea != null) {
			String[] informacion = linea.split(";");
			String habitacionID = informacion[0];
			Habitacion habitacion = habitaciones.get(habitacionID);
			for(int i = 1; i<=11; i++) {
				String caracteristica = informacion[i];
				String[] info = caracteristica.split(",");
				String nombre = info[0];
				String disponibilidad = info[1];
				boolean disponible = Boolean.parseBoolean(disponibilidad);
				CaracteristicasAdicionales atributo = new CaracteristicasAdicionales(nombre, disponible);
				habitacion.addCaracteristica(atributo);
			}
			linea = br.readLine();
		}
		br.close();
	...
	private void cargarCaracteristicas() throws IOException {
		BufferedReader br = new BufferedReader(new FileReader(archivoCaracteristicasHotel));
		String linea = br.readLine();
		while (linea != null) {
			String[] informacion = linea.split(";");
			String nombre = informacion[0];
			String disponibilidad = informacion[1];
			boolean disponible = Boolean.parseBoolean(disponibilidad);
			CaracteristicasAdicionales caracteristica = new CaracteristicasAdicionales(nombre, disponible);
			caracteristicasHotel.add(caracteristica);
			linea = br.readLine();
		}
		br.close();
	}
